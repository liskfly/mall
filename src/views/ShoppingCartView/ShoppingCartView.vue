...
export default {
  ...
  methods: {
        ...
        if (checked == true) {
          this.Total =
            ...
            parseFloat(
              parseFloat(
                this.goodscar[index].value *
                  parseFloat(this.goodscar[index].price)
              ).toFixed(2)
            );
        }
      });
    ...
    allCheck() {
      ...
      this.$refs.fff.forEach((item) => {
        item.checked = this.$refs.isAllCheck.checked ? true : false;
      });
      this.getTotal();
    },
    listCheck() {
      ...
      this.$refs.isAllCheck.checked =
        num > 0 && this.$refs.fff.length > 1 ? false : true;
      this.getTotal();
    },
    deleteGoods(index) {
    ...
    addPayment() {
      ...
      let car = this.goodscar.filter((item, i) => {
        if (this.$refs.fff[i].checked == true) {
          data.push({...item,id:new Date().getTime()});
          return false;
        } else {
          return true;
        }
